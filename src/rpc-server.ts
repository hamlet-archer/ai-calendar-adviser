/**
 * Unix-socket RPC server for the calendar-adviser. Newline-delimited JSON
 * envelopes in, newline-delimited JSON responses out.
 *
 * Wire format:
 *   request line  = JSON object matching either `contracts/calendar.query.v1.json`
 *                   or `contracts/calendar.find_free_slot.v1.json` (the
 *                   `contract_id` field selects the schema).
 ...
 *     bounded to 1 MiB to defend against unbounded inputs.
...
// Bound per-connection line buffer to 1 MiB. Anything larger is a client
// defect or an attack; close the connection rather than grow unboundedly.
...
function handleLine(
  ...
  try {
    let response: object;
    if (envelope.contract_id === 'calendar.query.v1') {
      response = handleCalendarQuery(envelope, {
        cache: deps.cache,
        calendarIds: deps.calendarIds,
      });
    } else if (envelope.contract_id === 'calendar.find_free_slot.v1') {
      response = handleFindFreeSlot(envelope, {
        cache: deps.cache,
        calendarIds: deps.calendarIds,
      });
    } else {
