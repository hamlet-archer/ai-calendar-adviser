/**
 * One-time per-user OAuth consent bootstrap (G6.5b).
 *
 ...
 * (`src/google-calendar-user-oauth-adapter.ts`) expects.
 *
 ...
 *     "allowed_scopes":   ["https://www.googleapis.com/auth/calendar.readonly"],
 *     "granted_at":       "<ISO timestamp>",
 *     "granted_subject":  "ai@liao.info"
 *   }
 *
 * Single subject (`ai@liao.info`) — calendar-adviser does NOT need a
 ...
 *   /etc/ai-calendar-adviser/oauth-token.json (mode 0600, owner ai-calendar-adviser).
 *
 * Client reuse: the row says "Reuses the existing per-user OAuth client
 * (`951896555491-88slv13200i...` from §G2)". Calendar scope needs adding
 * to that client's consent screen in Google Cloud Console before this
...
function parseArgs(argv: readonly string[]): Args {
  ...
  if (!resolvedScopes.includes(CALENDAR_READONLY_SCOPE)) {
    throw new Error(
      `--scopes must include calendar.readonly (got ${resolvedScopes.join(',')}); ` +
...
async function main(): Promise<void> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      'GOOGLE_OAUTH_CLIENT_ID and GOOGLE_OAUTH_CLIENT_SECRET must be set in env. ' +
        ...
        'client_id + client_secret. Calendar.readonly must be added to that client\'s ' +
  ...
  process.stdout.write(
    `\nWrote ${outPath} (mode 0600).\n` +
      `Next: scp this file to golden-ai-ops at /etc/ai-calendar-adviser/oauth-token.json ` +
      `(mode 0600, owner ai-calendar-adviser).\n`,
  );
}
